#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
#include <iostream>
#include <cstdio>
#include <cmath>
#include <cstring>
#include <algorithm>
#include <iomanip>
#include <chrono>
#include <immintrin.h>
using namespace std;
using namespace std::chrono;
typedef int64_t int64;
typedef unsigned int uint;

const int64 m1 = 12289;
const int64 m2 = 40961;
const int64 m3 = 65537;
const int64 m4 = 114689;
const int64 M1 = (int64)m2 * m3 * m4;
const int64 M2 = (int64)m1 * m3 * m4;
const int64 M3 = (int64)m1 * m2 * m4;
const int64 M4 = (int64)m1 * m2 * m3;
const int64 m1_ = 11711;
const int64 m2_ = 37278;
const int64 m3_ = 20721;
const int64 m4_ = 94134;
const int64 mod = m1 * m2 * m3 * m4;
const int64 G = 23;
const int64 Gi_1 = 6946;
const int64 Gi_2 = 21371;
const int64 Gi_3 = 45591;
const int64 Gi_4 = 9973;
const int TEST_TIMES = 4000;

int64 wn_rec[4097];
__m256i wn_rec_avx[4097];
__m256i m_avx;// = { m1,m2,m3,m4 };
__m256i limit_inv;// = { 12286,40951,65521,114661 };

const int64 deg = 2048;
int64 res[TEST_TIMES];
int64 limit[TEST_TIMES]; //
int64 L[TEST_TIMES];         // 二进制的位数
int64 RR[2 * deg];
int64 a[TEST_TIMES][deg], b[TEST_TIMES][deg];
int64 a_in[TEST_TIMES][2 * deg], b_in[TEST_TIMES][2 * deg];
int64 ret_1[TEST_TIMES][deg], ret_2[TEST_TIMES][deg], ret_3[TEST_TIMES][deg], ret_4[TEST_TIMES][deg];

__m256i a_avx[TEST_TIMES][deg];
__m256i b_avx[TEST_TIMES][deg];
__m256i a_in_avx[TEST_TIMES][2 * deg];
__m256i b_in_avx[TEST_TIMES][2 * deg];
__m256i ret_avx[TEST_TIMES][deg];

int64 modpow(int64 a, int64 b, int64 mod)
{
    int64 res = 1;
    while (b)
    {
        if (b & 1)
            res = res * a % mod;
        a = a * a % mod;
        b >>= 1;
    }
    return res % mod;
}

int64 inv(int64 x, int64 mod)
{
    return modpow(x, mod - 2, mod);
}

void NTT(__m256i* A, int64 type, int64 G, __m256i mod, int index)
{
    for (int i = 0; i < limit[index]; ++i)
        if (i < RR[i])
            swap(A[i], A[RR[i]]);
    for (int mid = 1; mid < limit[index]; mid <<= 1)
    { // 原根代替单位根
        int64 step = 2048 / mid;
        for (int len = mid << 1, pos = 0; pos < limit[index]; pos += len)
        {
            for (int k = 0; k < mid; k++)
            {
                __m256i w;
                if (type == 1)
                    w = wn_rec_avx[step * k];
                else
                    w = wn_rec_avx[step * (2 * mid - k)];
                __m256i x = A[pos + k];
                __m256i y;// = w * A[pos + mid + k] % mod;
                y = _mm256_mul_epi32(w, A[pos + mid + k]);
                y.m256i_i64[0] %= mod.m256i_i64[0];
                y.m256i_i64[1] %= mod.m256i_i64[1];
                y.m256i_i64[2] %= mod.m256i_i64[2];
                y.m256i_i64[3] %= mod.m256i_i64[3];
                A[pos + k] = _mm256_add_epi64(x, y);// (x + y);
                A[pos + k + mid] = _mm256_sub_epi64(x, y);// ((x - y));
            }
        }
    }
    if (type == -1)
    {
        //int64 limit_inv = inv(limit, mod);
        for (int i = 0; i < limit[index]; ++i)
            A[i] = _mm256_mul_epi32(A[i], limit_inv);//(A[i] * limit_inv);//% mod;
    }
}
void poly_mul(int index)
{
    memset(a_in_avx[index], 0, sizeof(a_in_avx[index]));
    memset(b_in_avx[index], 0, sizeof(b_in_avx[index]));
    memcpy(a_in_avx[index], a_avx[index], deg * sizeof(__m256i));
    memcpy(b_in_avx[index], b_avx[index], deg * sizeof(__m256i));
    for (limit[index] = 1, L[index] = 0; limit[index] <= deg; limit[index] <<= 1)
        L[index]++;
    for (int i = 0; i < limit[index]; ++i)
    {
        RR[i] = (RR[i >> 1] >> 1) | ((i & 1) << (L[index] - 1));
    }

    NTT(a_in_avx[index], 1, G, m_avx, index);
    NTT(b_in_avx[index], 1, G, m_avx, index);
    for (int i = 0; i < limit[index]; ++i)
    {
        a_in_avx[index][i] = _mm256_mul_epi32(a_in_avx[index][i], b_in_avx[index][i]);
        a_in_avx[index][i].m256i_i64[0] %= m_avx.m256i_i64[0];
        a_in_avx[index][i].m256i_i64[1] %= m_avx.m256i_i64[1];
        a_in_avx[index][i].m256i_i64[2] %= m_avx.m256i_i64[2];
        a_in_avx[index][i].m256i_i64[3] %= m_avx.m256i_i64[3];
        ;// a_in[i] * b_in[i] % m_avx;
    }
    NTT(a_in_avx[index], -1, G, m_avx, index);

    __m256i mod_24 = _mm256_slli_epi64(m_avx, 24);

    for (int i = 0; i < deg; i++)
    {
        __m256i temp = _mm256_sub_epi64(a_in_avx[index][i], a_in_avx[index][deg + i]);
        __m256i temp2 = _mm256_add_epi64(temp, mod_24);
        ret_avx[index][i].m256i_i64[0] = temp2.m256i_i64[0] % m_avx.m256i_i64[0];
        ret_avx[index][i].m256i_i64[1] = temp2.m256i_i64[1] % m_avx.m256i_i64[1];
        ret_avx[index][i].m256i_i64[2] = temp2.m256i_i64[2] % m_avx.m256i_i64[2];
        ret_avx[index][i].m256i_i64[3] = temp2.m256i_i64[3] % m_avx.m256i_i64[3];
    }
}

int main()
{
    //初始化全局变量
    m_avx.m256i_i64[0] = m1;
    m_avx.m256i_i64[1] = m2;
    m_avx.m256i_i64[2] = m3;
    m_avx.m256i_i64[3] = m4;
    limit_inv.m256i_i64[0] = 12286;
    limit_inv.m256i_i64[1] = 40951;
    limit_inv.m256i_i64[2] = 65521;
    limit_inv.m256i_i64[3] = 114661;
    //初始化根数组
    for (int i = 0; i < 4097; i++)
    {
        wn_rec_avx[i].m256i_i64[0] = modpow(G, (m1 - 1) * i / 4096, m1);
        wn_rec_avx[i].m256i_i64[1] = modpow(G, (m2 - 1) * i / 4096, m2);
        wn_rec_avx[i].m256i_i64[2] = modpow(G, (m3 - 1) * i / 4096, m3);
        wn_rec_avx[i].m256i_i64[3] = modpow(G, (m4 - 1) * i / 4096, m4);
    }
    for (int z = 0; z < TEST_TIMES; z++)
    {
        for (int i = 0; i < deg; ++i)
        {
            for (int j = 0; j < 4; j++)
            {
                a_avx[z][i].m256i_i64[j] = 1;
                b_avx[z][i].m256i_i64[j] = 1;
            }
        }
    }
    auto start = high_resolution_clock::now();
#pragma omp parallel for
    for (int j = 0; j < TEST_TIMES; j++)
        poly_mul(j);
    auto end = high_resolution_clock::now();
    cout << "用时：" << duration_cast<milliseconds>(end - start).count() << "ms" << endl;
    for (int i = 0; i <= 10; ++i)
    {
        cout << setw(6) << m1 - ret_avx[int(TEST_TIMES * 0.2222)][i].m256i_i64[0];
    }
    cout << endl;
    for (int i = 0; i <= 10; ++i)
    {
        cout << setw(6) << m2 - ret_avx[int(TEST_TIMES * 0.4444)][i].m256i_i64[1];
    }
    cout << endl;
    for (int i = 0; i <= 10; ++i)
    {
        cout << setw(6) << m3 - ret_avx[int(TEST_TIMES * 0.6666)][i].m256i_i64[2];
    }
    cout << endl;
    for (int i = 0; i <= 10; ++i)
    {
        cout << setw(6) << m4 - ret_avx[int(TEST_TIMES * 0.8888)][i].m256i_i64[3];
    }
    cout << endl;
}